...
import { jobs, alumni } from '../../data/staticData';
import { Search, Filter, MapPin, Building2, Calendar, Clock, Users, Bookmark, ExternalLink } from 'lucide-react';
...
const JobSearch = () => {
  ...
  const allJobs = [
    ...jobs,
    // Adding more job opportunities from alumni
    {
      id: 3,
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`${getApplicantColor(job.applications)}`}>
                      {job.applications} applicants
                    </span>
                  </div>
                </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <h4 className="font-medium text-gray-800 mb-2">Network First</h4>
            <p className="text-sm text-gray-600">Connect with alumni at target companies before applying.</p>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-3">
              <ExternalLink className="h-6 w-6 text-white" />
            </div>
            <h4 className="font-medium text-gray-800 mb-2">Tailor Resume</h4>
          ...
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            ...
            <h4 className="font-medium text-gray-800 mb-2">Apply Early</h4>
            <p className="text-sm text-gray-600">Submit applications within the first week of posting.</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {companies.slice(0, 5).map((company) => {
            ...
            const avgSalary = companyJobs.reduce((sum, job) => {
              // Extract number from salary string for calculation
              const salaryMatch = job.salary.match(/\$(\d+)/);
              return sum + (salaryMatch ? parseInt(salaryMatch[1]) : 0);
            ...
            return (
              <div key={company} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <div className="text-sm text-gray-600">{companyJobs.length} open positions</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium text-green-600">
                    ${avgSalary > 1000 ? `${Math.round(avgSalary/1000)}K avg` : `$${Math.round(avgSalary)}/hr avg`}
                  </div>
                  <div className="text-xs text-gray-500">
                    {companyJobs.reduce((sum, job) => sum + job.applications, 0)} total applicants
                  </div>
                </div>
              </div>
            );
